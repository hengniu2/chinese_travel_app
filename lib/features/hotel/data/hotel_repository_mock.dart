...
/// Replace with [HotelRepositoryApi] or similar when integrating real API.
class HotelRepositoryMock implements HotelRepository {
  HotelRepositoryMock({
    this.simulateNoInternet = false,
  ...
  /// Exposed for sync use cases (e.g. home carousel). Prefer [getHotels] for list/detail.
  static List<Hotel> get hotelsForDisplay => List.unmodifiable(_hotels);

  /// Sync lookup for order/legacy flows. Prefer [getHotelById] for UI.
  static Hotel? getHotelByIdSync(String id) {
    try {
  ...
  Future<HotelListResult> getHotels(HotelListQuery query) async {
    ...
    if (simulateNoInternet) {
      return HotelListResult(
          hotels: [], hasMore: false, error: HotelListError.noInternet);
    }
    if (simulateApiError) {
      return HotelListResult(
          hotels: [], hasMore: false, error: HotelListError.apiError);
    }

    var list = List<Hotel>.from(_hotels);

    if (query.searchKeyword != null &&
        query.searchKeyword!.trim().isNotEmpty) {
      final k = query.searchKeyword!.trim().toLowerCase();
      list = list
          .where((h) =>
              h.name.toLowerCase().contains(k) ||
    ...
    if (query.priceMin != null) {
      list = list.where((h) => h.price >= query.priceMin!).toList();
    }
    if (query.priceMax != null) {
      list = list.where((h) => h.price <= query.priceMax!).toList();
    }
    if (query.star != null) {
      list = list.where((h) => h.star == query.star).toList();
    }
      ...
      Hotel(
        ...
        rooms: [
          Room(
              ...
              capacity: 2,
              price: 2680,
              cancellationPolicy: '入住前1天18:00前免费取消',
